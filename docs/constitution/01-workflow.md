...
Documents first, code last. Each phase has a `/command` in `.claude/commands/` (for Claude Code) or `.devin/workflows/` (for Devin) — plain markdown, so an AI without slash-commands can open the file and follow it. The user drives the project by running these in order; every command ends with **you** deciding the next step, and every transition is written into `docs/status.md`.

## Day one — the documents

| # | Phase | Command | Produces |
|---|-------|---------|----------|
| 0 | Start — language, profile, git | `/start` | `docs/profile.md`, `docs/status.md` seeded |
| 1 | Idea core + competitor advantage + scope + deps | `/idea-core` | `docs/versions/0.1/01-idea-core.md` |
| 2 | PRD | `/prd` | `docs/living/prd.md` |
| 3 | Data model — the things the product remembers | `/data-model` | `docs/living/data-model.md` |
| 4 | UX — screens, flows, theme | `/ux` | `docs/living/ux.md` (may back-edit the PRD and the data model) |
| 5 | Design — the visual screens | `/design` | `UI/`, `docs/versions/0.1/design-chat.md` |
...
| # | Phase | Command | Produces |
|---|-------|---------|----------|
| 6 | Reconcile — make the documents agree again | `/reconcile` | updated living documents, `docs/decisions.md` |
| 7 | Dependencies — make each one usable | `/deps` | `docs/living/dependencies.md`, `secrets/secrets.local.md` filled |
| 8 | Test scenarios — the acceptance contract | `/scenarios` | `docs/living/test-scenarios.md` |
| 9 | Plan — architecture, checklist, handoff | `/plan` | `architecture.md`, `checklist.md`, `handoff.md` |
| 10 | Prepare the development environment | `/setup-dev` | the toolchain the plan needs, installed and verified |
| 11 | Build — the code, then running locally | `/build` | the code, the app running on `http://localhost:<port>` |
| 12 | Deploy — put it on the internet | `/deploy` | a live public address, recorded in `docs/profile.md` |
| 13 | Acceptance testing — the human rounds | `/test` | numbered test-round files, fixes |
| 14 | Iterate — 0.2, 0.3, … | `/iterate` | a new version folder, changes applied, app redeployed |

**Every new version starts in a fresh session, without exception.** A new version is a thinking step and the previous version's conversation is dead weight. Say that before anything else.
...
- **Planning and building are two commands, two sessions.** Everything that needs judgement happens in `/plan`, while the whole product conversation is still in context. It ends by writing `handoff.md` and telling the user to start a fresh session and run `/build`. The fresh session is the point: the build gets a full context window, and it gets it because the plan is on disk rather than in the conversation. `/build` executes and does not redesign.
...
- **`/build` will not start** until `docs/versions/<v>/handoff.md`, `docs/versions/<v>/checklist.md` and `docs/living/architecture.md` all exist. If any is missing, the planning step has not run — say so and stop, rather than improvising it.
...
4. **Plan, then build.** Write this version's `checklist.md` and `handoff.md` in the current session. Then look at what you just wrote and **offer** the session split instead of instructing it: if the checklist has **eight items or fewer and adds no new external dependency**, say the version is small enough to finish in this session; otherwise recommend starting a fresh session and running `/build`. Say which case applies and why, in one line.
